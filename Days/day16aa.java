...
public class day16aa {
	...
	public static int bigMethod(int duration, int depth) throws Exception {
		...
		while(minutes > 0){
			minutes--;
			//String save = current.name;
			Valve save = current;
			current = bestWithSkipsConsidered(current, minutes, depth);
			System.out.print("\n" + (30 - minutes) + " " + current.name + " " + current.value);
			if(save == current) { //save.equals(current.name)) {
				minutes--;
	...
	public static Valve bestWithSkipsConsidered(Valve start, int minutes, int depth) {
		int open = (start.value * (minutes - 1)) + comp(start, minutes - 1, depth);
		int skip = comp(start, minutes, depth + 2);
		
		if(skip > open) return bestPathForDepth(start, minutes, depth);
		return start;
	}
